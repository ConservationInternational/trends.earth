...
class SubnationalPolygonMapTool(QgsMapToolDigitizeFeature):
    """Polygon capture tool for defining subnational analysis unit boundaries.

    Emits ``polygon_captured(QgsGeometry)`` when the user finishes drawing a
    polygon (right-click or double-click to close).  Does NOT add any feature
    to a vector layer — a lightweight in-memory layer is used solely to
    satisfy the base-class requirements.
    """

    polygon_captured = QtCore.pyqtSignal(object)  # QgsGeometry

    def __init__(self, canvas: QgsMapCanvas):
        # A minimal in-memory polygon layer is needed so the base class
        # cadCanvasReleaseEvent does not bail out on a null currentVectorLayer().
        self._temp_layer = QgsVectorLayer(
            "Polygon?crs=EPSG:4326", "_subnational_boundaries_temp", "memory"
        )
        # The base class checks isEditable() before accepting any vertex clicks.
        self._temp_layer.startEditing()
        super().__init__(
            canvas, iface.cadDockWidget(), QgsMapToolCapture.CapturePolygon
        )
    ...
    # Override the virtual digitized() so the base class never tries to add
    # the feature to a real layer or open a feature form.
    def digitized(self, feature: QgsFeature):
        geom = feature.geometry()
        self.stopCapturing()
        self.cadDockWidget().clear()
        # Roll back so the temp layer stays empty, then restart for next draw
        if self._temp_layer.isEditable():
            self._temp_layer.rollBack()
        self._temp_layer.startEditing()
        if not geom.isEmpty() and geom.area() > 0:
