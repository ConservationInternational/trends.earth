...
class TCSummary(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        # Inputs
        self.addParameter(
            QgsProcessingParameterFile("INPUT", self.tr("Input carbon analysis file"))
        )
        self.addParameter(
            QgsProcessingParameterNumber("year_initial", self.tr("Starting year"))
        )
        self.addParameter(
            QgsProcessingParameterNumber("year_final", self.tr("Ending year"))
        )
        ...
        self.addOutput(
            QgsProcessingOutputNumber("CARBON_INITIAL", self.tr("Initial tonnes of C"))
        )
        self.addOutput(
            QgsProcessingOutputNumber("AREA_FOREST", self.tr("Area of forest in sq km"))
        )
        self.addOutput(
            QgsProcessingOutputNumber(
                "AREA_NON_FOREST", self.tr("Area of non-forest in sq km")
            )
        )
        self.addOutput(
            QgsProcessingOutputNumber(
        ...
        self.addOutput(
            QgsProcessingOutputNumber("AREA_WATER", self.tr("Area of water in sq km"))
        )
        self.addOutput(
            QgsProcessingOutputNumber("AREA_SITE", self.tr("Area of site in sq km"))
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
            ...
            for x in range(0, xsize, x_block_size):
                ...
                for n in range(year_end - year_start):
                    ...
                    forest_loss[n] = forest_loss[n] + np.sum(
                        (f_loss_array == year_start - 2000 + n + 1) * cell_areas_array
                    )
                    # Check units here - is tc_array in per m or per ha?
                    carbon_loss[n] = carbon_loss[n] + np.sum(
                        (f_loss_array == year_start - 2000 + n + 1)
                        * tc_array
                        * (tc_array >= 0)
                        * cell_areas_array
                    )
        ...
        return {
            "FOREST_LOSS": np.array2string(forest_loss),
