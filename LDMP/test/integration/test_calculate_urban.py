...
URBAN_TESTDATA = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__), "fixtures/Surinam_Paramaribo_11_3_1_Urban_10km.tif"
    )
)
...
class DlgCalculateUrbanSummaryTableWorkerTests(unittest.TestCase):
    def setUp(self):
        ...
        self.d.area_tab.area_admin_0.setCurrentIndex(
            self.d.area_tab.area_admin_0.findText("Suriname")
        )
        self.d.area_tab.radioButton_secondLevel_city.setChecked(True)
        self.d.area_tab.groupBox_buffer.setChecked(True)
        self.d.area_tab.secondLevel_city.setCurrentIndex(
            self.d.area_tab.secondLevel_city.findText("Paramaribo (Paramaribo)")
        )
    ...
    def testBands(self):
        ...
        self.assertEqual(
            self.d.combo_layer_urban_series.currentText(), "Urban area change"
        )
