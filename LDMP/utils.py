...
def qgis_process_path() -> str:
    ...
    elif platform_name == "darwin" or platform_name in ("linux", "freebsd"):
...
class FileUtils:
    ...
    def report_templates_dir() -> str:
        return os.path.normpath(f"{FileUtils.plugin_dir()}/data/reports")

    @staticmethod
    def project_path_from_report_task(report_task, root_output_dir: str) -> str:
    ...
    @staticmethod
    def open_project(report_task, root_output_dir: str) -> bool:
        """
    ...
    def get_icon(icon_name: str) -> QtGui.QIcon:
        ...
        icon_path = os.path.normpath(f"{FileUtils.plugin_dir()}/icons/{icon_name}")
