...
te_logo = Image.open(
    os.path.join(
        pathlib.Path(__file__).parent.absolute(), "trends_earth_logo_bl_print.png"
    )
)
...
def landtrend_get_data(year_start, year_end, geojson):
    ...
    prec = ee.Image(precip).select(
        ee.List.sequence(year_start - 1981, year_end - 1981, 1)
    )
...
def landtrend_make_plot(d, year_start, year_end):
    ...
    spec = gridspec.GridSpec(
        ncols=1, nrows=3, figure=fig, height_ratios=[3, 3, 1], hspace=0
    )
    ...
    axs[0].set_ylim(d.ndvi.min() * 0.95, d.ndvi.max() * 1.1)
    axs[0].set_ylabel(_("NDVI\n(annual)"), fontsize=32)
    axs[0].tick_params(
        axis="y", left=False, right=True, labelleft=False, labelright=True, labelsize=28
    )
...
def base_image(year, geojson, lang, gc_client, metadata):
    ...
    map_l8sr_mosaic = ee.ImageCollection.fromImages(
        [
            map_l8sr,
            ee.Image()
            .int()
    ...
    l8sr_url = map_l8sr_mosaic.getThumbUrl(
        {"region": region.getInfo(), "dimensions": 256}
    )
    l8sr_name = "l8sr.png"
...
def greenness(year, geojson, lang, gc_client, metadata):
    ...
    map_mean_mosaic = ee.ImageCollection.fromImages(
        [
            map_mean,
            ee.Image()
            .int()
    ...
    # create thumbnail and retrieve it by url
    mean_url = map_mean_mosaic.getThumbUrl(
        {"region": region.getInfo(), "dimensions": 256}
    )
    ...
    about = metadata["greenness"]["about"][lang].format(
...
def greenness_trend(year_start, year_end, geojson, lang, gc_client, metadata):
    ...
    map_trnd_mosaic = ee.ImageCollection.fromImages(
        [
            map_trnd,
            ee.Image()
            .int()
    ...
    # create thumbnail and retrieve it by url
    trnd_url = map_trnd_mosaic.getThumbUrl(
        {"region": region.getInfo(), "dimensions": 256}
    )
    ...
    about = metadata["greenness_trend"]["about"][lang].format(
        YEAR_START=start_date.strftime("%Y/%m/%d"),
        YEAR_END=end_date.strftime("%Y/%m/%d"),
    )
    out = ImageryPNG(
...
def run(params, logger):
    ...
    with open(
        os.path.join(pathlib.Path(__file__).parent.absolute(), "landpks_infotext.json")
    ) as f:
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        res.append(
            executor.submit(
                landtrend, year_start, year_end, geojson, lang, gc_client, metadata
            )
        )
        res.append(
            executor.submit(base_image, year_end, geojson, lang, gc_client, metadata)
        )
        res.append(
            executor.submit(greenness, year_end, geojson, lang, gc_client, metadata)
        )
        res.append(
            executor.submit(
                greenness_trend,
