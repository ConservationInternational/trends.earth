...
def run_te_for_period(params, EXECUTION_ID, logger):
    ...
    # Use unified percentile calculation across all geojson areas
    out.merge(
        productivity_performance(
            prod_params.get("perf_year_initial"),
            prod_params.get("perf_year_final"),
            prod_asset,
            all_geojsons,  # Pass all geojsons for unified percentile calculation
            logger,
        )
    )
...
def run_period(params, EXECUTION_ID, logger):
    ...
    if (
        ...
        params["productivity"]["mode"] == ProductivityMode.JRC_5_CLASS_LPD.value
        or params["productivity"]["mode"] == ProductivityMode.FWV2_5_CLASS_LPD.value
    ):
        ...
        # TODO: This function still uses ThreadPoolExecutor - should be updated like the others
        out = _run_faowocat_for_period(params, 4, EXECUTION_ID, logger)
    else:
        raise Exception(
...
def run(params, logger):
    ...
    return run_period(params, EXECUTION_ID, logger)
