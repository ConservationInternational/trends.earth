...
def urban(
    isi_thr, ntl_thr, wat_thr, cap_ope, pct_suburban, pct_urban, un_adju, crs,
    geojsons, EXECUTION_ID, logger
):
    # Impervious surface index computed by Trends.Earth
    isi_series = ee.ImageCollection(
        "projects/trends_earth/isi_1998_2018_20190403"
    ).reduce(ee.Reducer.mean()).select(
        [
            'isi1998_mean', 'isi2000_mean', 'isi2005_mean', 'isi2010_mean',
            'isi2015_mean', 'isi2018_mean'
    ...
    ntl = ntl.filterDate(ee.Date("2015-01-01"),
                         ee.Date("2015-12-31")).select(["avg_rad"], [
                             "ntl"
                         ]).median().clip(
                             ee.Geometry.Polygon(
                                 [
                                     -180, 57, 0, 57, 180, 57, 180, -88, 0,
                                     -88, -180, -88
                                 ], None, False
                             )
                         ).unmask(10)
    ...
    urban98 = isi_series.select("isi1998").gte(isi_thr).unmask(0).where(
        ntl.lte(-1 + ntl_thr * 31 / 100), 0
    )
    urban00 = isi_series.select("isi2000").gte(isi_thr).unmask(0).where(
        ntl.lte(-1 + ntl_thr * 31 / 100), 0
    )
    urban05 = isi_series.select("isi2005").gte(isi_thr).unmask(0).where(
        ntl.lte(-1 + ntl_thr * 31 / 100), 0
    )
    urban10 = isi_series.select("isi2010").gte(isi_thr).unmask(0).where(
        ntl.lte(-1 + ntl_thr * 31 / 100), 0
    )
    urban15 = isi_series.select("isi2015").gte(isi_thr).unmask(0).where(
        ntl.lte(-1 + ntl_thr * 31 / 100), 0
    )
    urban18 = isi_series.select("isi2018").gte(isi_thr).unmask(0).where(
        ntl.lte(-1 + ntl_thr * 31 / 100), 0
    )

    urban_series = (urban98.multiply(100000)).add(urban00.multiply(10000)).add(
        urban05.multiply(1000)
    ).add(urban10.multiply(100)).add(urban15.multiply(10)
                                     ).add(urban18.multiply(1))

    urban_sum = urban98.add(urban00).add(urban05).add(urban10).add(
        urban15
    ).add(urban18)
    ...
    if un_adju:
        ...
        # Density
        gpw4_2000 = ee.Image(
            "CIESIN/GPWv4/unwpp-adjusted-population-density/2000"
        ).select(["population-density"],
                 ["p2000"]).reproject(crs=proj, scale=30)
        gpw4_2005 = ee.Image(
            "CIESIN/GPWv4/unwpp-adjusted-population-density/2005"
        ).select(["population-density"],
                 ["p2005"]).reproject(crs=proj, scale=30)
        gpw4_2010 = ee.Image(
            "CIESIN/GPWv4/unwpp-adjusted-population-density/2010"
        ).select(["population-density"],
                 ["p2010"]).reproject(crs=proj, scale=30)
        gpw4_2015 = ee.Image(
            "CIESIN/GPWv4/unwpp-adjusted-population-density/2015"
        ).select(["population-density"],
                 ["p2015"]).reproject(crs=proj, scale=30)
    else:
    ...
    urban_series = urban_series.where(urban_series.eq(0), 0).where(
        urban_series.eq(1), 0
    ).where(urban_series.eq(10), 1).where(urban_series.eq(11), 4).where(
        urban_series.eq(100), 0
    ).where(urban_series.eq(101), 3).where(urban_series.eq(110), 3).where(
        urban_series.eq(111), 3
    ).where(urban_series.eq(1000), 0).where(
        urban_series.eq(1001), 2
    ).where(urban_series.eq(1010), 2).where(
        urban_series.eq(1011), 2
    ).where(urban_series.eq(1100), 2).where(
        urban_series.eq(1101), 2
    ).where(urban_series.eq(1110), 2).where(
        urban_series.eq(1111), 2
    ).where(urban_series.eq(10000), 0).where(urban_series.eq(10001), 0).where(
        urban_series.eq(10010), 0
    ).where(urban_series.eq(10011), 1).where(urban_series.eq(10100), 0).where(
        urban_series.eq(10101), 1
    ).where(urban_series.eq(10110), 1).where(urban_series.eq(10111), 1).where(
        urban_series.eq(11000), 0
    ).where(urban_series.eq(11001), 1).where(urban_series.eq(11010), 1).where(
        urban_series.eq(11011), 1
    ).where(urban_series.eq(11100), 1).where(urban_series.eq(11101), 1).where(
        urban_series.eq(11110), 1
    ).where(urban_series.eq(11111),
            1).where(urban_series.gte(100000).And(
                urban_sum.gte(3)
            ), 1).where(urban_series.gte(100000).And(urban_sum.lte(2)),
                        0).where(water.gte(wat_thr),
                                 -1).reproject(crs=proj, scale=30)
    ...
    def f_city_zones(built_up, geojson):
        ...
        city = ee.Image(10).where(
            dens.lte(pct_suburban).And(built_up.eq(1)), 3
        ).where(dens.gt(pct_suburban).And(built_up.eq(1)),
                2).where(dens.gt(pct_urban).And(built_up.eq(1)), 1)
        ...
        open_space = city.updateMask(city.eq(10)
        ...
        return city.where(city.eq(4).And(
            water.gte(wat_thr)
        ), 7).where(city.eq(5).And(water.gte(wat_thr)),
                    8).where(city.eq(6).And(water.gte(wat_thr)),
                             9).where(city.eq(10), -32768)
    ...
    for geojson in geojsons:
        ...
        city05 = f_city_zones(
            urban_series.gte(1).And(urban_series.lte(2)), geojson
        )
        city10 = f_city_zones(
            urban_series.gte(1).And(urban_series.lte(3)), geojson
        )
        city15 = f_city_zones(
            urban_series.gte(1).And(urban_series.lte(4)), geojson
        )
        rast_export = city00.addBands(city05).addBands(city10).addBands(
            city15
        ).addBands(gpw4_2000).addBands(gpw4_2005).addBands(gpw4_2010).addBands(
            gpw4_2015
        ).addBands(urban_series)
        rast_export = rast_export.unmask(-32768).int16().reproject(
            crs=proj, scale=30
        )
        this_out = TEImage(
            rast_export, [
        ...
        outs.append(
            this_out.export([geojson], 'urban', crs, logger, EXECUTION_ID)
        )
...
def run(params, logger):
    ...
    if area > 35000:
        logger.debug(
            "Area ({:.6n} km sq) is too large - failing task".format(area)
        )
        raise Exception
    else:
    ...
    out = urban(
        isi_thr, ntl_thr, wat_thr, cap_ope, pct_suburban, pct_urban, un_adju,
