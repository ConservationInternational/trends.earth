...
def urban(isi_thr, ntl_thr, wat_thr, crs, geojsons, EXECUTION_ID, logger):
    ...
    for geojson in geojsons:
        print(geojson)
        print('asdffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff')
        ...
        rast_export = urban_series.addBands(city00).addBands(city05).addBands(city10).addBands(city15).addBands(gpw4_2000).addBands(gpw4_2005).addBands(gpw4_2010).addBands(gpw4_2015)
...
GEOJSON = """[{"type": "FeatureCollection", "features": [{"geometry": {"type": "Polygon", "coordinates": [[[-172.70348059782563, -13.783993762276168], [-172.31843014206856, -13.783993762276168], [-172.31843014206856, -13.58458356218172], [-172.70348059782563, -13.58458356218172], [-172.70348059782563, -13.783993762276168]]]}}]}]"""
...
def run(params, logger):
    ...
    isi_thr = params.get('isi_thr', 20)
    ntl_thr = params.get('ntl_thr', 2)
    wat_thr = params.get('wat_thr', 25)
    geojsons = json.loads(params.get('geojsons', GEOJSON))
    ...
    out = urban(isi_thr, ntl_thr, wat_thr, crs, geojsons, EXECUTION_ID, logger)
